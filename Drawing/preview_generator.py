...
def write_preview(output_path, paths, config):
    ...
    if len(points_mm) > 1:
        points_px = [
            (
                int(round((x / film_w) * (width_px - 1))),
                int(round((y / film_h) * (height_px - 1)))
            )
        ...
        draw.line(
            points_px,
            fill="black",
            width=max(1, int(config.preview_line_width_px)),
            joint="curve"
        )
    ...
    return {
        ...
        "preview_width_px": width_px,
        "preview_height_px": height_px,
    }
