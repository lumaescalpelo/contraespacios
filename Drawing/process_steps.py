...
from svg_generator import _partial_landscape_bands, _internal_feature_lines, _contours_to_paths, _nearest_order
...
def save_process_steps(session, image_analysis, env_stats, visual, config, output_dir):
    ...
    add_step(1, contact_name, 'Hoja de contacto de las fotos válidas utilizadas en la sesión.')

    # 02 session + environment summaries
    session_summary_name = '02_session_summary.json'
    save_json(steps_dir / session_summary_name, {
    ...
    add_step(2, session_summary_name, 'Resumen de archivos de entrada usados para esta generación.')

    env_summary_name = '03_environment_summary.json'
    save_json(steps_dir / env_summary_name, {
        'environment_stats': env_stats,
        'visual_parameters': visual,
    })
    add_step(3, env_summary_name, 'Estadísticas ambientales y parámetros visuales derivados de las lecturas.')

    # 04-07 raster stages
    name = '04_composite_gray.png'
    _save_gray_like(image_analysis['gray'], steps_dir / name)
    add_step(4, name, 'Composición en gris creada a partir de varias fotos de la sesión.')

    name = '05_smoothed.png'
    _save_gray_like(image_analysis['smooth'], steps_dir / name)
    add_step(5, name, 'Imagen suavizada para reducir ruido y preparar extracción de formas.')

    name = '06_quantized.png'
    _save_gray_like(image_analysis['quantized'], steps_dir / name)
    add_step(6, name, 'Imagen cuantizada en niveles tonales para simplificar las masas visuales.')

    name = '07_contours_overlay.png'
    _save_contours_overlay(image_analysis, steps_dir / name)
    add_step(7, name, 'Contornos detectados superpuestos sobre la composición en gris.')
    ...
    add_step(8, name, 'Bandas de paisaje parciales generadas a partir de la lectura tonal horizontal.')

    name = '09_internal_lines_preview.png'
    write_preview(steps_dir / name, internal_paths, config)
    add_step(9, name, 'Líneas internas suaves que sugieren estructura y variación local.')

    name = '10_contour_paths_preview.png'
    write_preview(steps_dir / name, contour_paths, config)
    add_step(10, name, 'Trayectorias derivadas de los contornos principales detectados en la imagen.')
    ...
    add_step(11, name, 'Vista previa del recorrido final continuo, ya ordenado para dibujo sin eje Z.')

    meta_name = '12_path_groups_metadata.json'
    save_json(steps_dir / meta_name, {
        'band_meta': band_meta,
    ...
    add_step(12, meta_name, 'Metadatos de las familias de trayectorias y del ordenamiento final.')

    manifest_name = '00_manifest.json'
    save_json(steps_dir / manifest_name, {'steps': manifest})

    readme_name = 'README_steps.txt'
    lines = ['Proceso de transformación de imagen para Contra Espacios:', '']
    for item in manifest:
        lines.append(f"{item['step']:02d}. {item['file']} - {item['description']}")
